...
import com.fhss.shop.utils.FontUtil;
import com.fhss.shop.utils.PixelUtil;

import java.util.ArrayList;

/**
 *  by Administrator on 2017/8/14.
 */

public class ShapeButton extends LinearLayout {
    public static final String TAG = ShapeButton.class.getSimpleName();

    private Context mContext;

    // # Background Attributes
    private int mDefaultBackgroundColor 		= Color.BLACK;// 背景色
    private int mFocusBackgroundColor 			= 0;// 点击时背景色
    ...
    private int mDefaultTextSize 				= PixelUtil.INSTANCE.sp2px(getContext(), 15);
    private int mDefaultTextGravity             = 0x11; // Gravity.CENTER
    private String mText 						= null;

    // # Icon Attributes
    private Drawable mIconResource 				= null;
    private int  mFontIconSize 					= PixelUtil.INSTANCE.sp2px(getContext(), 15);
    ...
    private String mDefaultIconFont = "phone.ttf";
    private String mDefaultTextFont = "phone.ttf";
    ...
    public ShapeButton(Context context){
        ...
        mTextTypeFace   = FontUtil.findFont(mContext, mDefaultTextFont, null);
        mIconTypeFace   = FontUtil.findFont(mContext, mDefaultIconFont, null);
        initializeButton();
    }
    ...
    public ShapeButton(Context context, AttributeSet attrs){
        ...
        TypedArray attrsArray 	= context.obtainStyledAttributes(attrs, R.styleable.ShapeButton, 0, 0);
    ...
    private TextView setupFontIconView(){
        ...
        if(mFontIcon!=null){
            ...
            if(!isInEditMode()){
                fontIconView.setTextSize(PixelUtil.INSTANCE.px2sp(getContext(), mFontIconSize));
...
//                iconViewParams.leftMargin = 10;
    ...
    private void initAttributesArray(TypedArray attrsArray){
        ...
        mDefaultBackgroundColor 		= attrsArray.getColor(R.styleable.ShapeButton_fb_defaultColor,mDefaultBackgroundColor);
        mFocusBackgroundColor 			= attrsArray.getColor(R.styleable.ShapeButton_fb_focusColor,mFocusBackgroundColor);
        mDisabledBackgroundColor        = attrsArray.getColor(R.styleable.ShapeButton_fb_disabledColor, mDisabledBackgroundColor);

        mEnabled                        = attrsArray.getBoolean(R.styleable.ShapeButton_android_enabled, true);

        mDisabledTextColor              = attrsArray.getColor(R.styleable.ShapeButton_fb_disabledTextColor, mDisabledTextColor);
        mDisabledBorderColor            = attrsArray.getColor(R.styleable.ShapeButton_fb_disabledBorderColor, mDisabledBorderColor);
        mDefaultTextColor 				= attrsArray.getColor(R.styleable.ShapeButton_fb_textColor,mDefaultTextColor);
        // if default color is set then the icon's color is the same (the default for icon's color)
        mDefaultIconColor               = attrsArray.getColor(R.styleable.ShapeButton_fb_iconColor,mDefaultTextColor);

        mDefaultTextSize                = (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_textSize, mDefaultTextSize);
        mDefaultTextSize                = (int) attrsArray.getDimension(R.styleable.ShapeButton_android_textSize, mDefaultTextSize);

        mDefaultTextGravity             = attrsArray.getInt(R.styleable.ShapeButton_fb_textGravity, mDefaultTextGravity);

        mBorderColor 					= attrsArray.getColor(R.styleable.ShapeButton_fb_borderColor, mBorderColor);
        mBorderWidth					= (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_borderWidth,mBorderWidth);

        mRadius 						= (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_radius,mRadius);

        mRadiusTopLeft                  = (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_radiusTopLeft, mRadius);
        mRadiusTopRight                 = (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_radiusTopRight, mRadius);
        mRadiusBottomLeft               = (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_radiusBottomLeft, mRadius);
        mRadiusBottomRight              = (int) attrsArray.getDimension(R.styleable.ShapeButton_fb_radiusBottomRight, mRadius);

        mFontIconSize 					= (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_fontIconSize, mFontIconSize);

        mIconPaddingLeft                = (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_iconPaddingLeft,mIconPaddingLeft);
        mIconPaddingRight               = (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_iconPaddingRight,mIconPaddingRight);
        mIconPaddingTop                 = (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_iconPaddingTop,mIconPaddingTop);
        mIconPaddingBottom              = (int)attrsArray.getDimension(R.styleable.ShapeButton_fb_iconPaddingBottom,mIconPaddingBottom);

        mTextAllCaps                    = attrsArray.getBoolean(R.styleable.ShapeButton_fb_textAllCaps, false);
        mTextAllCaps                    = attrsArray.getBoolean(R.styleable.ShapeButton_android_textAllCaps, false);

        mGhost                          = attrsArray.getBoolean(R.styleable.ShapeButton_fb_ghost, mGhost);
        mUseSystemFont                  = attrsArray.getBoolean(R.styleable.ShapeButton_fb_useSystemFont, mUseSystemFont);
        mIsSingleLine                  = attrsArray.getBoolean(R.styleable.ShapeButton_fb_isSingleLine, mIsSingleLine);

        String text = attrsArray.getString(R.styleable.ShapeButton_fb_text);

        if (text == null) { //no fb_text attribute
            text = attrsArray.getString(R.styleable.ShapeButton_android_text);
        }

        mIconPosition 					= attrsArray.getInt(R.styleable.ShapeButton_fb_iconPosition,mIconPosition);

        String fontIcon 				= attrsArray.getString(R.styleable.ShapeButton_fb_fontIconResource);

        String iconFontFamily 			= attrsArray.getString(R.styleable.ShapeButton_fb_iconFont);
        String textFontFamily 			= attrsArray.getString(R.styleable.ShapeButton_fb_textFont);

        Drawable icon = null;
        try{
            mIconResource 				= attrsArray.getDrawable(R.styleable.ShapeButton_fb_iconResource);
        ...
        if(!isInEditMode()){
            if(iconFontFamily!=null){
                mIconTypeFace = FontUtil.findFont(mContext, iconFontFamily, mDefaultIconFont);
            }else{
                mIconTypeFace = FontUtil.findFont(mContext, mDefaultIconFont, null);
            }

            if(textFontFamily!=null){
                mTextTypeFace = FontUtil.findFont(mContext, textFontFamily, mDefaultTextFont);
            }else{
                mTextTypeFace = FontUtil.findFont(mContext, mDefaultTextFont, null);
            }
        }
    ...
    public void setTextSize(int textSize){
        this.mDefaultTextSize = PixelUtil.INSTANCE.sp2px(getContext(), textSize);
        if(mTextView != null)
    ...
    public void setFontIconSize(int iconSize){
        this.mFontIconSize = PixelUtil.INSTANCE.sp2px(getContext(), iconSize);
        if(mFontIconView!=null)
    ...
    public void setCustomTextFont(String fontName){
        mTextTypeFace = FontUtil.findFont(mContext, fontName, mDefaultTextFont);
    ...
    public void setCustomIconFont(String fontName){
        ...
        mIconTypeFace = FontUtil.findFont(mContext, fontName, mDefaultIconFont);
